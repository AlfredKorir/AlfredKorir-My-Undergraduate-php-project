...
class Staff extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['county']=$this->Home_model->select_countries();
    ...
    public function insert()
    {
        ...
        if($this->form_validation->run() !== false)
        {
            ...
            if($login>0)
            {
                $data=$this->Staff_model->insert_staff(array('id'=>$login,'staff_name'=>$name,'gender'=>$gender,'email'=>$email,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'city'=>$city,'state'=>$state,'county'=>$country,'department_id'=>$department,'pic'=>$image,'added_by'=>$added));
            }
    ...
    public function update()
    {
        ...
        if($this->form_validation->run() !== false)
        {
            ...
            if ( ! $this->upload->do_upload('filephoto'))
            {
                $data=$this->Staff_model->update_staff(array('staff_name'=>$name,'gender'=>$gender,'email'=>$email,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'city'=>$city,'state'=>$state,'county'=>$country,'department_id'=>$department),$id);
            }
            else
            {
                $image_data =   $this->upload->data();
                ...
                $data=$this->Staff_model->update_staff(array('staff_name'=>$name,'gender'=>$gender,'email'=>$email,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'city'=>$city,'state'=>$state,'county'=>$country,'department_id'=>$department,'pic'=>$image_data['file_name'],'added_by'=>$added),$id);
            }
